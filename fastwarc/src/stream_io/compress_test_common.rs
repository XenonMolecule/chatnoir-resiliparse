...
use crate::stream_io::{BufReadSeek, CompressingStream, DecompressingStream};
...
pub(crate) fn test_reader_new_read_seek_and_stream_position<C, R, S>(compress_fn: C, reader_new_fn: R) -> io::Result<()>
where
...
pub(crate) fn test_reader_with_capacity_fill_buf_consume_and_into_inner<C, R, S, I>(
    compress_fn: C,
...
pub(crate) fn test_reader_with_tiny_capacity_handles_many_refills<C, R, S>(
    compress_fn: C,
    reader_with_capacity_fn: R,
) -> io::Result<()>
where
...
pub(crate) fn test_reader_reads_to_eof_after_external_compression<C, R, S>(
    compress_fn: C,
    reader_new_fn: R,
) -> io::Result<()>
where
...
pub(crate) fn test_reader_inner_seek_inner_stream_position_and_member_tracking<C, R, S>(
    compress_fn: C,
    reader_new_fn: R,
) -> io::Result<()>
where
...
pub(crate) fn test_writer_new_write_and_into_inner_roundtrip<D, W, S, I>(
    decompress_fn: D,
    writer_new_fn: W,
    into_inner_fn: I,
) -> io::Result<()>
...
pub(crate) fn test_writer_drop_finishes_and_flushes_stream<D, W, S>(
    decompress_fn: D,
...
pub(crate) fn test_writer_propagates_inner_flush_errors<W, S>(writer_with_capacity_fn: W) -> io::Result<()>
where
...
pub(crate) fn test_writer_propagates_inner_write_errors<W, S>(writer_with_capacity_fn: W) -> io::Result<()>
where
    W: Fn(usize, ErrorWriter) -> S,
    S: CompressingStream,
{
